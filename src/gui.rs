...
use super::{Pools, Player, gamelog::GameLog, Map, Name, Position, State, InBackpack, Viewshed, RunState, Equipped, HungerState, HungerClock, Hidden, SHOW_MAPGEN_VISUALIZER, camera, Attribute, Attributes, Consumable};
...
pub fn draw_ui(ecs: &World, ctx : &mut Rltk) {
    use rltk::to_cp437;
    let box_gray : RGB = RGB::from_hex("#999999").expect("Entered incorrect hex code for RGB at gui.rs");
    let black = RGB::named(rltk::BLACK);
    let white = RGB::named(rltk::WHITE);
    ...
    match hc.state {
        ...
        HungerState::Normal => ctx.print_color(50,44, RGB::named(rltk::WHITE), RGB::named(rltk::BLACK), "Satisfied"),
...
pub fn show_inventory(gs : &mut State, ctx : &mut Rltk) -> (ItemMenuResult, Option<Entity>) {
    ...
    //WARNING: too many inventory items leads to overflow
    //TODO add pages to inventory menu
...
pub fn main_menu(gs : &mut State, ctx : &mut Rltk) -> MainMenuResult {
    ...
    ctx.print_color_centered(21, RGB::named(rltk::CYAN), RGB::named(rltk::BLACK), "by S.O");
...
// Same as from the RLTK library but without being filled in
...
pub fn game_over(ctx : &mut Rltk) -> GameOverResult {
    ...
    ctx.print_color_centered(18, RGB::named(rltk::WHITE), RGB::named(rltk::BLACK), "That day, sadly, is not today..");
