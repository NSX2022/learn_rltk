...
use crate::map_builders::{level_builder, BuilderMap, MetaMapBuilder};
use crate::{OtherLevelPosition, Position, Viewshed};
use super::{Map, TileType};

// Contains a HashMap of all previous maps, acts as a "Master map" that is basically the World of the game
// TODO introduce idx functions to allow 2D/3D structure and access if/when open world development is started
...
fn transition_to_new_map(ecs : &mut World, new_depth: i32) -> Vec<Map> {
    ...
    if new_depth > 1 {
        if let Some(pos) = &builder.build_data.starting_position {
            ...
            eprintln!("Placed upstairs");
            builder.build_data.map.tiles[up_idx] = TileType::UpStairs;
        }else{
            panic!("Builder starting position does not exist @dungeon.rs")
        }
    }
