...
impl PrefabBuilder {
    ...
    fn char_to_map(&mut self, ch : char, idx: usize) {
        match ch {
            ...
            '^' => {
                self.map.tiles[idx] = TileType::Floor;
                self.spawn_list.push((idx, "Clamp Trap".to_string()));
            }
            '%' => {
                self.map.tiles[idx] = TileType::Floor;
    ...
    fn apply_room_vaults(&mut self) {
        ...
        let master_vault_list = vec![SILLY_SMILE, CHECKERBOARD, TOTALLY_NOT_A_TRAP];
        ...
        if !vault_positions.is_empty() {
            ...
            let height = self.map.height; // when we access `self` inside the `retain`.. erm yikes
