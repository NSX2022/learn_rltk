...
pub fn cull_dead_particles(ecs : &mut World, ctx : &Rltk) {
    ...
    for dead in dead_particles.iter() {
        ecs.delete_entity(*dead).expect("Unable to remove particle");
    }
}
...
impl<'a> System<'a> for ParticleSpawnSystem {
    ...
    fn run(&mut self, data : Self::SystemData) {
        ...
        for new_particle in particle_builder.requests.iter() {
            ...
            renderables.insert(p, Renderable{ fg: new_particle.fg, bg: new_particle.bg, glyph: new_particle.glyph, render_order: -127 }).expect("Unable to insert renderable");
